# main.py
import json
import argparse
from receipt_processor import file_io as io_mod
from receipt_processor import gpt

def process_directory(dirpath):
    """Process a directory of receipt images.

    Lists all files in the given directory, encodes each file as base64,
    sends it to the language model for information extraction, and collects
    the results in a dictionary.

    Args:
        dirpath: Path to the directory containing receipt images.

    Returns:
        A dictionary mapping filenames to extracted receipt data.
    """
    results = {}
    for name, path in io_mod.list_files(dirpath):
        image_b64 = io_mod.encode_file(path)
        data = gpt.extract_receipt_info(image_b64)
        results[name] = data
    return results

def main():
    """
    Command-line entry point for the receipt extraction program.

    Parses command-line arguments, processes the input directory, and
    optionally prints the extracted data as formatted JSON.
    """
    parser = argparse.ArgumentParser()
    parser.add_argument("dirpath")
    parser.add_argument("--print", action="store_true")
    args = parser.parse_args()

    data = process_directory(args.dirpath)
    if args.print:
        print(json.dumps(data, indent=2))

if __name__ == "__main__":
    main()

