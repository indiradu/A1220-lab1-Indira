...
def extract_receipt_info(image_b64):
    """Extract structured receipt information from an image.

    Sends a base64-encoded receipt image to the OpenAI API and requests
    extraction of the date, total amount, vendor, and category.

    Args:
        image_b64: Base64-encoded receipt image.

    Returns:
        A dictionary with keys: date, amount, vendor, and category.
    """
    prompt = f"""
